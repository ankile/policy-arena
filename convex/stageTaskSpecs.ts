...
export const upsert = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existing = await ctx.db
      ...
      .unique();
    // A version's vocabulary is immutable by contract (taxonomy_pins.json on
    // the Python side); a hash change on re-upsert means the pin discipline
    // was bypassed — refuse rather than silently repoint stored labels.
    if (existing && existing.taxonomy_hash !== args.taxonomy_hash) {
      throw new Error(
        `taxonomy_hash changed for ${args.task}@${args.taxonomy_version} ` +
          `(${existing.taxonomy_hash} -> ${args.taxonomy_hash}); bump the ` +
          "taxonomy_version instead of mutating an existing vocabulary"
      );
    }
    if (args.live) {
