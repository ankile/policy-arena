...
and writes all fields atomically. A matching SHA plus algorithm version is a
cache hit and skips the parquet read.
...
The script reads `POLICY_ARENA_TOKEN` or
`~/.config/sir/policy_arena_token`, waits for the Convex action, and exits
nonzero on an explicit `stats_status=error`. The browser never calls the Hugging
Face Dataset Server filter endpoint and never writes summary statistics.
